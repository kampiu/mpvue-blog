        ...
        methods: {
            init(id) {
                this.$http.get(api.getAtricleItem(id)).then((res) => {
                    console.log(res)
                    if(res.code === 200) {
                        this.article = res.result.data.data[0]
                    }
                    res.code === 200 && store.commit("upArt", res.result.data.data[0].art_content)
                    wx.setNavigationBarTitle({
                        title: res.result.data.data[0].art_title
                    })
                })
                let that = this
                if(this.openid !== null) {
            ...
            checkTheColl(id) {
                console.log(this.userInfo.art)
                if(this.userInfo.art.length === 0) {
                    return
                }
                this.userInfo.art.forEach((item, index) => {
                    if(item == id) {
                        this.colle = true
                        return
                    }
